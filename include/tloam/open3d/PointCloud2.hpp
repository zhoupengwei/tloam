...
#ifdef OPEN3D_VERSION_MAJOR
#define OPEN3D_VERSION_GE(major, minor, patch)                                 \
  ((OPEN3D_VERSION_MAJOR > (major)) ||                                         \
   (OPEN3D_VERSION_MAJOR == (major) && OPEN3D_VERSION_MINOR > (minor)) ||      \
   (OPEN3D_VERSION_MAJOR == (major) && OPEN3D_VERSION_MINOR == (minor) &&      \
    OPEN3D_VERSION_PATCH >= (patch)))
#endif
...
public:
  ...
  std::shared_ptr<PointCloud2> SelectByIndex(const std::vector<size_t> &indices,
  ...
  VoxelDownSampleAndTrace(double voxel_size, const Eigen::Vector3d &min_bound,
  ...
  /// \brief RandomDownSample applies a random sampling with uniform probability
  /// through random library by C++11. Based off Algorithm A from the paper
  /// "Faster Methods for Random Sampling" by Jeffrey Scott Vitter. The
  /// algorithm runs in O(N) and results as return
  ///
  ...
  /// \author Pengwei Zhou
  /// \param sample_num, the number of sample to total number of points in the
  /// pointcloud \param dummy, it is a dummy parameter and only used to
  /// distinguish function overloading.
  std::shared_ptr<PointCloud2> RandomDownSample(size_t sample_num,
                                                bool dummy) const;
  ...
  /// console.
  std::vector<int> ClusterDBSCAN(double eps, size_t min_points,
  ...
  SegmentPlane(const double distance_threshold = 0.01, const int ransac_n = 3,
  ...
  std::shared_ptr<PointCloud2> CreateFromVoxelGrid(const VoxelGrid &voxel_grid);
