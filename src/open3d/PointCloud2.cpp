...
bool PointCloud2::IsEmpty() const { return !HasPoints(); }
...
#if OPEN3D_VERSION_GE(0, 17, 0)
OrientedBoundingBox
PointCloud2::GetMinimalOrientedBoundingBox(bool robust) const {
...
PointCloud2 &PointCloud2::Translate(const Eigen::Vector3d &translation,
                                    bool relative) {
  TranslatePoints(translation, points_, relative);
  return *this;
}

PointCloud2 &PointCloud2::Scale(const double scale,
                                const Eigen::Vector3d &center) {
  ScalePoints(scale, points_, center);
  return *this;
}

PointCloud2 &PointCloud2::Rotate(const Eigen::Matrix3d &R,
                                 const Eigen::Vector3d &center) {
...
std::vector<double>
PointCloud2::ComputePointCloudDistance(const PointCloud2 &target) {
...
#pragma omp parallel for schedule(static)
  for (int i = 0; i < (int)points_.size(); i++) {
    ...
    if (kdtree.SearchKNN(points_[i], 1, indices, dists) == 0) {
      utility::LogDebug("[ComputePointCloudToPointCloudDistance] Found a point "
                        "without neighbors.");
      distances[i] = 0.0;
    } else {
...
PointCloud2 &PointCloud2::RemoveNonFinitePoints(bool remove_nan,
  ...
  for (size_t i = 0; i < old_point_num; i++) { // old index
    bool is_nan =
        remove_nan && (std::isnan(points_[i](0)) || std::isnan(points_[i](1)) ||
                       std::isnan(points_[i](2)));
    bool is_infinite = remove_infinite &&
                       (std::isinf(points_[i](0)) ||
                        std::isinf(points_[i](1)) || std::isinf(points_[i](2)));
...
std::shared_ptr<PointCloud2>
PointCloud2::SelectByIndex(const std::vector<size_t> &indices,
                           bool invert) const {
  ...
  if (!invert) {
    ...
    if (has_normals)
      output->normals_.reserve(k);
    if (has_colors)
      output->colors_.reserve(k);
    if (has_intensity)
      output->intensity_.reserve(k);
  } else {
    output->points_.reserve(m - k);
    if (has_normals)
      output->normals_.reserve(m - k);
    if (has_colors)
      output->colors_.reserve(m - k);
    if (has_intensity)
      output->intensity_.reserve(m - k);
  }
  ...
  for (size_t i = 0; i < m; i++) {
    if (mask[i]) {
      ...
      if (has_intensity)
        output->intensity_.push_back(intensity_[i]);
    }
  }
  utility::LogDebug("Pointcloud down sampled from {:d} points to {:d} points.",
...
public:
  AccumulatedPoint()
      : num_of_points_(0), point_(0.0, 0.0, 0.0), normal_(0.0, 0.0, 0.0),
...
public:
  void AddPoint(const PointCloud2 &cloud, size_t index, int cubic_index,
...
std::shared_ptr<PointCloud2>
PointCloud2::VoxelDownSample(double voxel_size) const {
  auto output = std::make_shared<PointCloud2>();
  if (voxel_size <= 0.0) {
...
std::tuple<std::shared_ptr<PointCloud2>, Eigen::MatrixXi,
           std::vector<std::vector<int>>>
PointCloud2::VoxelDownSampleAndTrace(double voxel_size,
  ...
  for (size_t i = 0; i < points_.size(); i++) {
    ...
    auto voxel_index =
        Eigen::Vector3i(int(floor(ref_coord(0))), int(floor(ref_coord(1))),
                        int(floor(ref_coord(2))));
    int cid = 0;
    for (int c = 0; c < 3; c++) {
...
std::shared_ptr<PointCloud2>
PointCloud2::UniformDownSample(size_t every_k_points) const {
  if (every_k_points == 0) {
...
std::shared_ptr<PointCloud2>
PointCloud2::RandomDownSample(double sampling_ratio) const {
...
std::shared_ptr<open3d::geometry::PointCloud2>
PointCloud2::RandomDownSample(size_t sample_num, bool dummy) const {
  if (sample_num > this->points_.size()) {
    utility::LogWarning("[RandomSample] find the sample number of points less "
                        "than the original cloud. it will be disabled");
  ...
  std::uniform_real_distribution<double> uniform_decimal(0.0,
                                                         1.0); // [0.0, 1.0)
  std::size_t i = 0;
  std::size_t index = 0;
  ...
  while (n > 0) {
    // Step 1: [Generate. U.] Generate a random variate U that is uniformly
    // distributed between 0 and 1 through random library
    double U = uniform_decimal(generator);
    // Step 2: [Test.] If N * U > n, go to step 4.
    if (size_t((double)N * U) <= n) {
      // Step 3: [Select.] Select the next record in the file for the sample,
      // and set n : = n - 1.
      reserve_indexs[i++] = raw_indexs[index];
      --n;
    }
    // Step 4: [Don't select.] Skip over the next record (do not include it in
    // the sample). Set N : = N - 1.
    --N;
    ++index;
    // If n > 0, then return to Step 1; otherwise, the sample is complete and
...
std::shared_ptr<PointCloud2>
PointCloud2::Crop(const AxisAlignedBoundingBox &bbox) const {
  if (bbox.IsEmpty()) {
    utility::LogError(
...
std::shared_ptr<PointCloud2>
PointCloud2::Crop(const OrientedBoundingBox &bbox) const {
  if (bbox.IsEmpty()) {
    utility::LogError(
        "[CropPointCloud] AxisAlignedBoundingBox either has zeros "
        "size, or has wrong bounds.");
...
PointCloud2::RemoveRadiusOutliers(size_t nb_points,
                                  double search_radius) const {
  if (nb_points < 1 || search_radius <= 0) {
    utility::LogError("[RemoveRadiusOutliers] Illegal input parameters,"
...
#pragma omp parallel for schedule(static)
  for (int i = 0; i < int(points_.size()); i++) {
    ...
    size_t nb_neighbors =
        kdtree.SearchRadius(points_[i], search_radius, tmp_indices, dist);
...
std::tuple<std::shared_ptr<PointCloud2>, std::vector<size_t>>
PointCloud2::RemoveStatisticalOutliers(size_t nb_neighbors,
...
PointCloud2::ComputeMeanAndCovariance() const {
  if (IsEmpty()) {
...
PointCloud2::ComputeConvexHull() const {
  return Qhull::ComputeConvexHull(points_);
}

std::tuple<std::shared_ptr<TriangleMesh>, std::vector<size_t>>
PointCloud2::HiddenPointRemoval(const Eigen::Vector3d &camera_location,
                                const double radius) const {
  if (radius <= 0) {
    utility::LogError("[HiddenPointRemoval] radius must be larger than zero.");
  }
  ...
  if (origin_vidx < visible_mesh->vertices_.size()) {
    ...
    for (size_t tidx = visible_mesh->triangles_.size(); tidx-- > 0;) {
      if (visible_mesh->triangles_[tidx](0) == (int)origin_vidx ||
        ...
        visible_mesh->triangles_.erase(visible_mesh->triangles_.begin() + tidx);
      } else {
        if (visible_mesh->triangles_[tidx](0) > (int)origin_vidx)
...
std::vector<WeightedEdge> Kruskal(std::vector<WeightedEdge> &edges,
                                  size_t n_vertices) {
  std::sort(edges.begin(), edges.end(), [](WeightedEdge &e0, WeightedEdge &e1) {
...
void PointCloud2::EstimateNormals(const KDTreeSearchParam &search_param,
                                  bool fast_normal_computation) {
  bool has_normal = HasNormals();
  if (!has_normal) {
...
void PointCloud2::OrientNormalsToAlignWithDirection(
    const Eigen::Vector3d &orientation_reference) {
  if (!HasNormals()) {
    utility::LogError("[OrientNormalsToAlignWithDirection] No normals in the "
...
void PointCloud2::OrientNormalsTowardsCameraLocation(
    const Eigen::Vector3d &camera_location) {
  if (!HasNormals()) {
    utility::LogError("[OrientNormalsTowardsCameraLocation] No normals in the "
                      "PointCloud. Call EstimateNormals() first.");
  }
#pragma omp parallel for schedule(static)
  for (int i = 0; i < (int)points_.size(); i++) {
...
void PointCloud2::OrientNormalsConsistentTangentPlane(size_t k) {
  if (!HasNormals()) {
    utility::LogError("[OrientNormalsConsistentTangentPlane] No normals in the "
                      "PointCloud. Call EstimateNormals() first.");
  }
  ...
  std::tie(delaunay_mesh, pt_map) =
      TetraMesh::CreateFromPointCloud(PointCloud(this->points_));
...
std::vector<int> PointCloud2::ClusterDBSCAN(double eps, size_t min_points,
                                            bool print_progress) const {
  KDTreeFlann kdtree(*this);

  // Precompute all neighbors.
  utility::LogDebug("Precompute neighbors.");
#if OPEN3D_VERSION_GE(0, 14, 0)
...
RANSACResult
EvaluateRANSACBasedOnDistance(const std::vector<Eigen::Vector3d> &points,
                              const Eigen::Vector4d plane_model,
                              std::vector<size_t> &inliers,
                              double distance_threshold, double error) {
  RANSACResult result;
...
PointCloud2::SegmentPlane(const double distance_threshold /* = 0.01 */,
...
std::shared_ptr<PointCloud2> CreatePointCloudFromFloatDepthImage(
    const Image &depth, const camera::PinholeCameraIntrinsic &intrinsic,
...
std::shared_ptr<PointCloud2> CreatePointCloudFromRGBDImageT(
    const RGBDImage &image, const camera::PinholeCameraIntrinsic &intrinsic,
    const Eigen::Matrix4d &extrinsic, bool project_valid_depth_only) {
  auto pointcloud = std::make_shared<PointCloud2>();
  Eigen::Matrix4d camera_pose = extrinsic.inverse();
  auto focal_length = intrinsic.GetFocalLength();
  auto principal_point = intrinsic.GetPrincipalPoint();
  ...
  for (int i = 0; i < image.depth_.height_; i++) {
    auto *p =
        (float *)(image.depth_.data_.data() + i * image.depth_.BytesPerLine());
    TC *pc =
...
std::shared_ptr<PointCloud2> PointCloud2::CreateFromDepthImage(
    const Image &depth, const camera::PinholeCameraIntrinsic &intrinsic,
    const Eigen::Matrix4d &extrinsic, double depth_scale, double depth_trunc,
    int stride, bool project_valid_depth_only) {
  ...
  if (depth.num_of_channels_ == 1) {
    if (depth.bytes_per_channel_ == 2) {
      ...
      return CreatePointCloudFromFloatDepthImage(
          *float_depth, intrinsic, extrinsic, stride, project_valid_depth_only);
    } else if (depth.bytes_per_channel_ == 4) {
      return CreatePointCloudFromFloatDepthImage(
          depth, intrinsic, extrinsic, stride, project_valid_depth_only);
    }
  }
  utility::LogError(
...
std::shared_ptr<PointCloud2>
PointCloud2::CreateFromVoxelGrid(const VoxelGrid &voxel_grid) {
  auto output = std::make_shared<PointCloud2>();
